...
def z_propagation(omega_sig,omega_idx,a,lp,lp_length,k_sig,k_pump,E_pump,dz,Leff):
    ...
    for z_idx in range(0,lp_length-1):
        ...
        k1=np.zeros(len(omega_sig),dtype=np.complex128)
        k2=np.zeros(len(omega_sig),dtype=np.complex128)
        k3=np.zeros(len(omega_sig),dtype=np.complex128)
        k4=np.zeros(len(omega_sig),dtype=np.complex128)
        ...
        for omega1_idx, omega in enumerate(omega_sig):
            ...
            k1[omega1_idx] = 1 / (Lnl * E0) * nonlin_term
        for omega1_idx, omega in enumerate(omega_sig):
            z_val = lp[z_idx]
            integrand = np.exp(1j * (k_pump[:, omega1_idx]-k_sig[omega1_idx]-k_sig[:]) * (z_val - Leff / 2 +dz/2)) * E_pump[:, omega1_idx] * np.conjugate(a[cur,:]+k1*dz/2)
            nonlin_term = np.sum(integrand) * domg
            k2[omega1_idx] = 1 / (Lnl * E0) * nonlin_term
        for omega1_idx, omega in enumerate(omega_sig):
            z_val = lp[z_idx]
            integrand = np.exp(1j * (k_pump[:, omega1_idx]-k_sig[omega1_idx]-k_sig[:]) * (z_val - Leff / 2 +dz/2)) * E_pump[:, omega1_idx] * np.conjugate(a[cur,:]+k2*dz/2)
            nonlin_term = np.sum(integrand) * domg
            k3[omega1_idx] = 1 / (Lnl * E0) * nonlin_term
        for omega1_idx, omega in enumerate(omega_sig):
            z_val = lp[z_idx]
            integrand = np.exp(1j * (k_pump[:, omega1_idx]-k_sig[omega1_idx]-k_sig[:]) * (z_val - Leff / 2 +dz)) * E_pump[:, omega1_idx] * np.conjugate(a[cur,:]+k3*dz)
            nonlin_term = np.sum(integrand) * domg
            k4[omega1_idx] = 1 / (Lnl * E0) * nonlin_term
            #--------------------------------------------
        a[nextt, :] = a[cur, :] + (k1+2*k2+2*k3+k4)/6 * dz
...
num_freq_pts=1000 # generating n+1 x n+1 matrix for the Green functions
num_z_pts=100 # number of points in the z direction for simulation
...
if __name__=='__main__':
    for i,val_i in enumerate(arr_p):
        for j,val_j in enumerate(arr):
            ...
            with open(folder_name+file_name+'_substitution_rk4.pickle','wb') as handle:
            ...
            plt.savefig(folder_name+"S "+'pump scale '+"{:.2f}".format(scale_p)+', signal scale '+"{:.2f}".format(scale)+'_'+str(num_freq_pts)+'_' + str(num_z_pts)+'_'+str(power_avg_in)+'W_'+"{:.2f}".format(scale_p)+'_'+"{:.2f}".format(scale)+'_'+str(wl)+'_substitution_rk4.png', format='png')
            #plt.show()
            ...
            plt.savefig(folder_name+"C "+'pump scale '+"{:.2f}".format(scale_p)+', signal scale '+"{:.2f}".format(scale)+'_'+str(num_freq_pts)+'_' + str(num_z_pts)+'_'+str(power_avg_in)+'W_'+"{:.2f}".format(scale_p)+'_'+"{:.2f}".format(scale)+'_'+str(wl)+'_substitution_rk4.png', format='png')
            #plt.show()
            ...
            plt.colorbar()
            plt.savefig(folder_name+"phasematching function "+'pump scale '+"{:.2f}".format(scale_p)+', signal scale '+"{:.2f}".format(scale)+'_'+str(num_freq_pts)+'_' + str(num_z_pts)+'_'+str(power_avg_in)+'W_'+"{:.2f}".format(scale_p)+'_'+"{:.2f}".format(scale)+'_'+str(wl)+'_substitution_rk4.png', format='png')
            #plt.show()
            ...
            plt.colorbar()
            plt.savefig(folder_name+"JSA "+'pump scale '+"{:.2f}".format(scale_p)+', signal scale '+"{:.2f}".format(scale)+'_'+str(num_freq_pts)+'_' + str(num_z_pts)+'_'+str(power_avg_in)+'W_'+"{:.2f}".format(scale_p)+'_'+"{:.2f}".format(scale)+'_'+str(wl)+'_substitution_rk4.png', format='png')
            figure_num+=4
            #plt.show()
            p.clear()
